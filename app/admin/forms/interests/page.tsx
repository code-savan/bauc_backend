...
interface Interest {
  ...
  budget_range: string;
  created_at: string;
  // ... other fields available but not shown in main table
}
...
export default function InterestList() {
  ...
  const fetchInterests = async () => {
    try {
      ...
      setError('Failed to load expressions of interest. Please try again later.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredInterests = interests.filter(interest => {
    ...
    const value = interest[searchField as keyof Interest];
  ...
  return (
    <div className="container mx-auto py-8 pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Expression of Interest</h1>
          <p className="text-sm text-gray-500 mt-1">Manage property interest submissions</p>
        </div>
                ...
                >
                  ...
                  <TableCell className="text-right">
                    <Button
                      ...
                      asChild
                    >
                      <Link href={`/admin/forms/interests/${interest.id}`}>
